 ...
 const handleGetStarted = () => {
  ...
  return (
    <nav className={`fixed w-full z-[100] transition-all duration-500 ${scrolled ? 'glass py-3' : 'bg-transparent py-6'}`}>
      <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-12">
        <div className="flex justify-between items-center">
          <Link to="/" className="flex items-center space-x-2 group">
            <Logo size="sm" className="group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500" />
            <span className="font-display text-xl font-black tracking-tighter text-white">DIGITALSIGHT</span>
          </Link>
          ...
          <div className="hidden lg:flex items-center space-x-6">
              ...
              {NAV_ITEMS.map((item) => (
                item.isExternal ? (
                  <a
                    ...
                    className="text-xs font-bold uppercase tracking-wider transition-all hover:text-white text-gray-400"
                  >
                    {item.label}
                  </a>
                ) : (
            ...
            <button 
              ...
              className="bg-white text-black hover:bg-blue-600 hover:text-white px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-2xl shadow-white/5"
            >
              Login to Portal
            </button>
          </div>
      ...
      {/* Mobile Nav Overlay */}
      <div className={`fixed inset-0 bg-black z-40 lg:hidden transition-all duration-700 ease-in-out ${isOpen ? 'opacity-100 pointer-events-auto translate-y-0' : 'opacity-0 pointer-events-none -translate-y-10'}`}>
